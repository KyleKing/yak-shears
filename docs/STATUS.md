...
**Typed notes and their benches (2026-08).** `/streams` renders the canal (three reaches, latch and command deck, undo holding the exact inverse of each write), `/habits` the practice bench (schedules, streaks, grace, makeup), `/lists` the reference rack, and `/benches` the hub over all three. Every write goes through `rewrite_frontmatter_field`, so a note round-trips byte for byte. `/streams` queries the frontmatter store; `/habits` and `/lists` still read the vault directly and move onto the engine next.
...
| Link extraction | ✅ | `[[wikilinks]]` and `#tags` indexed on save and on the vault scan; repeats collapse to one edge |
